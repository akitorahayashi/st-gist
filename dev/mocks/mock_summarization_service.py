...
class MockSummarizationService:
    ...
    async def summarize(self, text: str, max_chars: int = 10000) -> str:
        ...
        point2 = self._generate_point(text, "重要なポイント") if point1 != self._generate_point(text, "重要なポイント") else f"追加の{self._generate_fallback_point(text)}"
        point3 = self._generate_point(text, "特徴的な要素") if point1 != self._generate_point(text, "特徴的な要素") else f"詳細な{self._generate_fallback_point(text)}"
...
• {point3}"""
    ...
    def _generate_point(self, text: str, context: str) -> str:
        ...
        if "ai" in text.lower() and ("model" in text.lower() or "news" in text.lower()):
            ...
            return "使いやすいインターフェースと高速な読み込み時間を特徴とするウェブサイト"
        else:
